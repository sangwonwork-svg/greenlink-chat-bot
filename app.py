...
with st.sidebar:
    ...
    if learned_files:
        for i, name in enumerate(learned_files, 1):
            st.write(f"{i}. {name}")
    else:
        st.warning("학습된 파일이 없습니다.")
    
    st.divider()
    if st.button("🔄 파일 새로고침 (Reboot)"):
...
if prompt := st.chat_input("문서 내용에 대해 물어보세요."):
    ...
    with st.chat_message("assistant"):
        ...
        # 지식 베이스가 너무 클 경우를 대비해 최근 3만 자 정도만 우선 전달 (조절 가능)
        context_text = knowledge_base[:30000] 
        
        messages = [
            {
                "role": "system", 
                "content": f"너는 사내 문서 전문가야. 아래 제공된 문서 내용을 바탕으로 답변해줘. 한국어로 친절하게 답변하고, 문서에 없으면 모른다고 해.\n\n[문서 내용]\n{context_text}"
            }
        ]
